...
import type { AnalysisConstructorParams, Data, EmailData, SMSData, RunUserNotificationCreate } from "jsr:@tago-io/sdk";
...
async function startAnalysis(context: AnalysisConstructorParams, scope: Data[]): Promise<void> {
  ...
  if (email_tag) {
    ...
    await email_service.send(emailData)
  ...
  if (phone_tag) {
    ...
    await sms_service.send(smsData)
      .then((result) => console.log(result))
      .catch((error) => console.log(error));
  } else {
  ...
  if (userID_tag) {
    ...
    await Resources.run.notificationCreate(userID_tag.value, notificationData)
      .then((result) => console.log(result))
      .catch((error) => console.log(error));
  } else {
